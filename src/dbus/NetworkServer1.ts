 ...
 */
export class OrgBluezNetworkServer1 {
    public static readonly DbusInterfaceName = "org.bluez.NetworkServer1";
    ...
    ): Promise<OrgBluezNetworkServer1> {
        return bus.getProxyObject("org.bluez", objectPath, xml).then((obj) => new OrgBluezNetworkServer1(obj));
    }
    ...
    public Register(uuid: string, bridge: string): Promise<void> {
        return this.thisDBusInterface.Register(wrapDbusSignature(uuid, "s"), wrapDbusSignature(bridge, "s"));
    }

    //@method({ name: 'Unregister', inSignature: 's', outSignature: '' })
    public Unregister(uuid: string): Promise<void> {
        return this.thisDBusInterface.Unregister(wrapDbusSignature(uuid, "s"));
    }
}
